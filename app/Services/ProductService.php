...
class ProductService
{
    ...
    {
        $product = ProductModel::with('attributes')->where('id', $productId)->first();
        $success = [];
        $errors = [];
    ...
    public function updateAllQuantity()
    {
        try {
            ...
            foreach ($info as $item) {
                $attribute = ProductAttribute::where('code', $item['PROD_CD'])->first();
                if ($attribute) {
                    $attribute->update([
    ...
    public function sendToGoogleMerchant()
    {
        try {
            ...
            $products = ProductModel::where('gmc', 1)
                ->where('published', 1)
                ->with([
                    'category',
            ...
            foreach ($products as $item) {
                ...
                foreach ($item->attributes as $attribute) {
                    $value[] = [
                        ...
                        14 => '', // mpn
                        15 => $item->brand ?: '', // brand
                        16 => $hightlights, // product_highlight
                        17 => $detail, // product_detail
            ...
            if ($result->updates->updatedRows != 0) {
                ...
                // return response()->json(['message' => 'fail'], 200, [], JSON_PRETTY_PRINT);
    ...
    public function sendToGoogleMerchant_API()
    {
        ...
        foreach ($products as $product) {
            $googleProduct = new \Google\Service\ShoppingContent\Product([
                'offerId' => $product->id,
                'title' => $product->name,
                'description' => $product->description,
                'link' => $product->href,
                'imageLink' => asset($product->image),
                'contentLanguage' => 'vi',
                'targetCountry' => 'VN',
                'channel' => 'online',
                'price' => new \Google\Service\ShoppingContent\Price([
                    'value' => $product->price_public,
                    'currency' => 'VND',
                ]),
            ...
            try {
                ...
                $errors[] = $product->id . ', Error: ' . $e->getMessage();
            }
        }
