...
class Shopee implements EcommercePlatform
{
    ...
    function __construct()
    {
        ...
        $this->fastService = new FastService();
    }
    function authShop()
    {
    ...
    public function getOrders()
    {
        ...
        $time_from = strtotime('-3 days', $time_to);
        ...
        if ($ret['error'] == '') {
            DB::table('token_ecommerce')->where('type', 'shopee')->update([
                'more' => !empty($ret['response']['more']) ? 1 : 1,
            ...
            foreach ($ret['response']['order_list'] as $order) {
                ...
                $existingOrder = DB::table('shopee')->where('order_code', $order_sn)->first();

                $payment_response = $this->getEscrowDetail($order_sn, $token);
                if (empty($existingOrder->payment_response) || $payment_response['error'] == '') {
    ...
    public function getOrderAPI()
    {
        ...
        $perPage = 100;
        $orders = DB::table('shopee')->orderBy('created_at', 'desc')->paginate($perPage, ['*'], 'page', $page);
        return response()->json(['message' => 'success', 'data' => $orders], 200);
    }

    public function getOrderDetailAPI()
    {
        $order = DB::table('shopee')->where('order_code', request()->order_code)->first();
