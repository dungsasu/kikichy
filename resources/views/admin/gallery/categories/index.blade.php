...
@section('content')
    <div class="row gy-4">
        @if ($show_button_action)
            <x-button-action create="{{ route('create_gallery-categories') }}" :model="$model" />
        @endif
        <div class="col-12">
            <x-filter prefix="{{ $prefix }}" />
        </div>
        <div class="col-12">
            <div class="card">
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a style="width: 30px" class="dropdown-item p-1"
                                                href="{{ route('edit_gallery-categories', ['id' => $item->id]) }}">
                                                <i class="mdi mdi-pencil-outline me-1"></i>
                                            </a>
                                            <a style="width: 30px" class="dropdown-item p-1 delete-record"
                                                data-link="{{ route('delete_gallery-categories', ['id' => $item->id, 'agreeDelete' => $item->id, 'route' => 'admin.gallery.categories.index']) }}">
