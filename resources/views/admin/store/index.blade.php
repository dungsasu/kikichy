...
@section('content')
    ...
    <div class="row gy-4">
        @if (@$show_button_action)
            <x-button-action show="create,duplicate,active,unactive,delete" :view="$view" create="{{ route('admin.store.create') }}" :model="$model" />
        @endif
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="d-flex">
                                            ...
                                            href="{{ route('admin.store.edit', ['id' => $item->id]) }}">
                                            <i class="mdi mdi-pencil-outline me-1"></i>
                                        </a>
                                        <a style="width: 30px" class="dropdown-item p-1 delete-record"
                                            data-link="{{ route('admin.store.delete', ['id' => $item->id, 'agreeDelete' => $item->id, 'route' => 'admin.store.index']) }}">
