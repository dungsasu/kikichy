...
@php
    $title = @$data->id ? 'Chi tiết' : 'Thêm' . ' ' . ' thông số KT';
@endphp
@section('title', $title)
...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="py-3 mb-4">
            <span class="text-muted fw-light"></span> {{ @$data->id ? 'Chi tiết' : 'Thêm' }} thông số KT
        </h4>
                ...
                <div class="card-body pt-2 mt-1">
                    <form action={{ route($view . '.save') }} id="formAccountSettings" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mt-2 gy-4">
                            ...
                            <div class="col-md-6">
                                <select name="filter_category_id" class="form-select select2 form-select-sm">
                                    <option value="0">---Danh mục thông số KT---</option>
                                    @foreach ($category as $item)
                                        <option value="{{ $item->id }}" {{ @$data->filter_category_id == $item->id ? 'selected' : null }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
